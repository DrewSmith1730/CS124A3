//
// Created by drews on 10/15/2020.
//

#ifndef CS124_BINARYSEARCHTREE_H
#define CS124_BINARYSEARCHTREE_H
...
private:
    ...
    // TODO: make it so it stores the depth of the last node it visited
    // TODO: ^ pass an integer by reference into both the funcs and modify it inside
    // Helper recursive function to find a value in the tree.
    bool find(const Comparable &c, BinaryNode* n, int* dep) const {
        if (n == nullptr) {
        ...
        if (c < n->value) {
            ...
            *dep = *dep + 1;
            return find(c, n->leftChild, dep);
        }
        if (n->value < c) {
            // Value is greater than current node. Go to node's right child.
            // increment the depth value
            *dep = *dep + 1;
            return find(c, n->rightChild, dep);
        }
...
public:
    ...
    bool find(const Comparable &c, int* dep) const {
        // calls private helper function
        return find(c, root, dep);
    }
...
#endif //CS124_BINARYSEARCHTREE_H
